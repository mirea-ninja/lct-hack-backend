...
from constants import RepairType, Segment, Walls
from utils import get_address_cords
...
def _get_polygon_param(lat, lon, radius):
    polygon = [
        geodesic(kilometers=radius / 1000).destination(point=(lat, lon), bearing=angle)
...
def parse_analogs(address: str, search_params: SearchParams) -> pd.DataFrame:
    ...
    # Площадь кухни, м2
    df["kitchen_area"] = df["area"].apply(
        lambda x: float(x.split("/")[2]) if len(x.split("/")) == 3 else np.nan
    )
    ...
    df["rooms"] = df["rooms"].apply(
        lambda x: int(x.split(",")[0]) if "," in x else int(x.strip())
    )

    # Этаж. Указан в 'house'. Формат: "9/14, Панельный", где 9 - этаж, 14 - этажность дома
    df["floor"] = df["house"].apply(
        lambda x: int(x.split("/")[0]) if x != "nan" else np.nan
    )

    # Этажность дома
    df["floors"] = df["house"].apply(
        lambda x: int(x.split("/")[1].split(",")[0]) if x != "nan" else np.nan
    )
    ...
    df["wall_material"] = df["house"].apply(
