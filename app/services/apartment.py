...
class ApartmentService:
    ...
    async def create(db: AsyncSession, guid: UUID4, subid: UUID4, model: ApartmentCreate) -> ApartmentGet:
        apartment = await ApartmentRepository.create(db, guid, subid, model)
    ...
    async def get_all(
        ...
        apartments = await ApartmentRepository.get_all(db, guid, subid, offset=offset, limit=limit)
    ...
    async def update(db: AsyncSession, guid: UUID4, subid: UUID4, aid: UUID4, model: ApartmentCreate) -> ApartmentGet:
        apartment = await ApartmentRepository.update(db, guid, subid, aid, model)
        if apartment is None:
            raise HTTPException(404, "Квартира не найдена")
        return ApartmentGet.from_orm(apartment)
    ...
    async def patch(db: AsyncSession, guid: UUID4, subid: UUID4, aid: UUID4, model: ApartmentPatch) -> ApartmentGet:
        apartment = await ApartmentRepository.patch(db, guid, subid, aid, model)
        if apartment is None:
            raise HTTPException(404, "Квартира не найдена")
        return ApartmentGet.from_orm(apartment)
    ...
    async def delete(db: AsyncSession, guid: UUID4, subid: UUID4, aid: UUID4) -> Response(status_code=204):
        await ApartmentRepository.delete(db, guid, subid, aid)
